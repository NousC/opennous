-- People vs Leads: keep cold/scraped leads OUT of the People (contacts) page.
--
-- Before this, the `contacts` view returned EVERY person entity, so cold leads
-- imported into lead lists (and comment-only LinkedIn engagers) polluted the
-- People page. A person now shows in People only when there is a real
-- relationship signal:
--   * any observation from a non-scrape source (manual add, CRM sync, inbound
--     reply, meeting, calendar, etc.) — i.e. NOT just 'lead_list'/'apify_linkedin', or
--   * the pipeline advanced past the cold/engaged top of funnel (covers a
--     replied lead even when the reply was logged through the lead UI).
--
-- Column set and order are unchanged, so the contacts INSTEAD OF triggers keep
-- working. Only the WHERE filter is added.
